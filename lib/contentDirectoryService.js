...
var ContentDirectoryService = function(configuration) {
  ...
  Service.call(this, {
    ...
    scpdURL : "/service/contentdirectory/description.xml",
  ...
  this.addType("ContainerUpdateIDs", "string", "", [], null, true, 2, null, function(){
    var updateIds = self.updateIds;
    self.updateIds = {};
    var containerUpdateIDs = [];
    for (var key in updateIds){
        containerUpdateIDs.push(key, updateIds[key])
    }
    self.stateVars["ContainerUpdateIDs"].value = containerUpdateIDs.join(",");
  });
  this.addType("SystemUpdateID", "ui4", 0, [], null, true, 2);
  ...
  this.addType("SortCapabilities", "string", [ "dc:title", "upnp:genre", "upnp:artist", "upnp:author",
  ...
  this.addType("SearchCapabilities", "string",[ "dc:title", "upnp:genre", "upnp:artist", "upnp:author",
      "upnp:album", "upnp:rating" , "upnp:class"].join(','), []);
  this.addType("A_ARG_TYPE_Filter", "string");

  //this.addStateAction("GetServiceResetToken","ResetToken");
...
ContentDirectoryService.prototype._prepareAttributesFilter = function(
  ...
  filterExpression.split(',').forEach(function(token) {
    ...
    var newPrefix = sp[1];  //xmlns[prefixXmlns];
...
ContentDirectoryService.prototype.processSoap_Search = function(xml, request,
    ...
    // console.log(Util.inspect(node));
      ...
      debug("CDS: Search sortCriteria:"+sortCriteria+" browseFlag:"+browseFlag+
          " requestedCount:"+requestedCount+" ContainerID:"+objectId+
          " startingIndex:"+startingIndex);


      return this.responseSeach(response, request, objectId,
          attributesFilter, startingIndex, requestedCount, sortCriteria,
          searchCriteria, callback);
};

ContentDirectoryService.prototype.responseSeach = function(response,
  ...
  this.getNodeById(
          ...
          function(error, item) {
            ...
            if (!item) {
              return callback(710, "CDS: Browser Can not find item " + containerId);
            }
            ...
            if (item.refID) {
              self.getNodeById(item.refID, function(error, refItem) {
                ...
                refItem.filterChildNodes(refItem, null, filter, function(error, list){
                  if (error) {
...
ContentDirectoryService.prototype.processSoap_Browse = function(xml, request,
  ...
  debug("CDS: Browse sortCriteria:"+sortCriteria+" browseFlag:"+browseFlag+
  ...
  callback(402, "Unknown browseFlag '" + browseFlag + "'");
};
